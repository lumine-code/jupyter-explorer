...
class ExplorerPane {
  constructor({ document: domDocument = document } = {}) {
    ...
    this.mountComponent(domDocument);
  ...
  getDefaultFocusTarget() {
    return (
      this.element.querySelector("lumine-text-editor.explorer-expression") ||
  ...
  focusExpression = () => {
    const editor = this.element.querySelector("lumine-text-editor.explorer-expression");
    (editor || this.getDefaultFocusTarget())?.focus?.({ preventScroll: true });
  };

  mountComponent(domDocument = this.element.ownerDocument, surfaceState = null) {
    if (surfaceState?.gridState) explorerStore.pendingRestore = surfaceState.gridState;
    this.component = new Explorer({
      store: explorerStore,
  ...
  beginWindowSurfaceTransition() {
    const surfaceState = {
      gridState: explorerStore.activeGrid?.captureState?.() || null,
      plotLayout: this.component?.plot?.captureState?.() || null,
    };
    this.component?.destroy();
    this.component = null;
    this.element.replaceChildren();
    this._lastFocused = null;
    const rebuild = async (surface) => {
      if (this.destroyed || this.component) return;
      this.mountComponent(surface.document, surfaceState);
      await this.component.plot?.whenReady?.();
    };
    return {
