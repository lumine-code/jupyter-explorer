...
class ExplorerStore {
  ...
  load = (kernel, expression) => {
    ...
    // Data was asked for, so the grid takes focus once it has rendered —
    // the same hand-off a drill does. Without it the wrap never focuses,
    // and the seeded selection never draws.
    this.focusToken += 1;
    this._emitUpdate();
    this._fetch();
  };
