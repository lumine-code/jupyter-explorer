...
function activate() {
  subscriptions = new CompositeDisposable(
    ...
    atom.workspace.addOpener((uri) => (uri === EXPLORER_URI ? createPane() : undefined)),
...
async function load(kernel, expression) {
  ...
  // The load hands focus to the grid once the data has rendered; the
  // expression editor must not steal it back.
  return open({ focusExpression: false });
}

async function open({ focusExpression = true } = {}) {
  ...
  const item = await atom.workspace.open(EXPLORER_URI, { searchAllPanes: true });
  // With data on the way the grid takes focus when it renders; the editor is
  // only the landing place of an empty panel.
  if (focusExpression && !loadedData) {
